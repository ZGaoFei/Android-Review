...
```
内存溢出（Out of Memory）：系统会给每个APP分配内存也就是Heap Size值。当APP占用的内存加上我们申请的内存资源超过了Dalvik虚拟机的最大内存时就会抛出的Out Of Memory异常。即申请的内存超过了虚拟键提供的内存。
释放强引用、使用软引用或者弱引用
...
内存泄漏出现的场景：
	1、非静态内部类和匿名内部类Handler、Thread、Runnable、AsyncTask等持有外部类的引用，activity关闭，线程未完成造成内存泄漏。使用静态内部类，当Activity销毁是及时关闭线程销毁资源
...
ANR（Application Not Responding）：应用程序无响应
	...
	10s内前台广播的onReceive()没有执行完，后台为60s
	20s前台服务，后台服务200s没有执行完毕
	10s内ContentProvider的publish没有执行完
